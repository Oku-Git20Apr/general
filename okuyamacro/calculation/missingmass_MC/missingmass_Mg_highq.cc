...
double fmm_core_total(double *x, double *par){//with core excited states
  //double val = par[0] * TMath::Gaus(x[0],par[1],par[2],kTRUE);//s-orbit
  //val += par[3] * TMath::Gaus(x[0],par[4],par[5], kTRUE);//s-orbit core excited
  //val += par[6] * TMath::Gaus(x[0],par[7],par[8], kTRUE);//s-orbit core excited
  //val += par[9] * TMath::Gaus(x[0],par[10],par[11], kTRUE);//s-orbit core excited
  //val += par[12] * TMath::Gaus(x[0],par[13],par[14], kTRUE);//1p-orbit
  //val += par[15] * TMath::Gaus(x[0],par[16],par[17], kTRUE);//1p-orbit core excited
  //val += par[18] * TMath::Gaus(x[0],par[19],par[20], kTRUE);//1p-orbit core excited
  //val += par[21] * TMath::Gaus(x[0],par[22],par[23], kTRUE);//1p-orbit core excited
  //val += par[24] * TMath::Gaus(x[0],par[25],par[26], kTRUE);//1p-orbit core excited
  //val += par[27] * TMath::Gaus(x[0],par[28],par[29], kTRUE);//2p-orbit
  //val += par[30] * TMath::Gaus(x[0],par[31],par[32], kTRUE);//p-orbit core excited
  //val += par[33] * TMath::Gaus(x[0],par[34],par[35], kTRUE);//p-orbit core excited
  //val += par[36] * TMath::Gaus(x[0],par[37],par[38], kTRUE);//p-orbit core excited
  //val += par[39] * TMath::Gaus(x[0],par[40],par[41], kTRUE);//p-orbit core excited
  //val += par[42] * TMath::Gaus(x[0],par[43],par[44], kTRUE);//p-orbit core excited
  //val += par[45] * TMath::Gaus(x[0],par[46],par[47], kTRUE);//p-orbit core excited
  double val = 0.;
	for(int i=0;i<19;i++){
...
//Energy loss inside Al cell
double dEdx_inside(double Delta, double thick, double Minc, double mom){ // dE, target thick, mass, momentum
  gErrorIgnoreLevel = kError;
...
//  double W;                              // maximum energy transfer in a single collision
  ...
  a = 0.1;
  m = 3.0;
  int tar = 5; // C:1, Ca40:2, Scin:3, Al:4, He:5
  switch(tar){
  case 1:  // C
    ro = 1.80;  Z = 6;  A = 12.011;
    I = 78 * 1e-6;  C0 = -2.99;
  ...
  case 4: // Al (Ref. Leo text)
    ...
    X1 = 3.01;  X0 = 0.1708;
    break;
  case 5: // He4 (At. Data and Nucl. Data Tables 30, 262 (1984).)
    //ro = 1.6632E-4;  
    ro = 0.0095;//3He at 12 K
    //ro = 0.0131;//4He at 12 K
    Z = 2;  A = 3;
    Ne = Na * ro * Z / A;  mup = sqrt(80.617 * 1e+6 * Ne);
    I = 41.8 * 1e-6;  C0 = -11.1393;
    a = 0.13443;  m = 5.8347;
    X1 = 3.6122;  X0 = 2.2017;
    break;
  default:
...
ROOT::Math::IntegratorOneDimOptions::SetDefaultRelTolerance(1.E-6); 
	...
	TFile *file_new = new TFile("250keV_single_p532_noscale.root","recreate");
...
//					1/2+(s)  1/2-,3/2-(p) 125 nb/sr -> divided by three.
  ...
  double CSpval1 = CSp_tot*5./10.;
  double CSpval2 = CSp_tot*3./10.;
  double CSpval3 = CSp_tot*2./10.;
  //CSpval1 = CSp_tot;
  //CSpval2 = 0.;
  //CSpval3 = 0.;
  double Expval1 = -6.3;
...
cout<<"CS_sum = "<<CS_sum<<" [nb/sr]"<<endl;
  ...
  double thick = 0.037*1.;  // target thickness in cm (0.05cm = 500um)
                         // E05  E12-15
...
cout<<"Acc = "<<Acc<<endl;
  ...
  TH1D *h1_gasdEe  = new TH1D("h1_gasdEe","dEe (gas)",400,0.,10.);
  TH1D *h1_gasdEep = new TH1D("h1_gasdEep","dEep (gas)",400,0.,10.);
  TH1D *h1_gasdEk  = new TH1D("h1_gasdEk","dEk (gas)",400,0.,10.);
  TH1D *h1_dEe  = new TH1D("h1_dEe","dEe",400,0.,10.);
  TH1D *h1_dEep = new TH1D("h1_dEep","dEep",400,0.,10.);
  TH1D *h1_dEk  = new TH1D("h1_dEk","dEk",400,0.,10.);
  TH1D *h1_2dEe = new TH1D("h1_2dEe","2dEe",400,0.,10.);
  TH1D *h1_2dEep= new TH1D("h1_2dEep","2dEep",400,0.,10.);
  TH1D *h1_2dEk = new TH1D("h1_2dEk","2dEk",400,0.,10.);
  TH2D *h2_dEep = new TH2D("h2_dEep","dEep",400,Theta_scat-0.005,Theta_scat+0.005,400,0.,10.);
  TH2D *h2_dEk  = new TH2D("h2_dEk","dEk",400,Theta_K-0.005,Theta_K+0.005,400,0.,10.);
  TH2D *h2_2dEep= new TH2D("h2_2dEep","2dEep",400,Theta_scat-0.005,Theta_scat+0.005,400,0.,10.);
  TH2D *h2_2dEk = new TH2D("h2_2dEk","2dEk",400,Theta_K-0.005,Theta_K+0.005,400,0.,10.);

//100keV bin
  TH1D *h1_mm_p = new TH1D("h1_mm_p","h1_mm_p",120,-20,10);
  TH1D *h1_mm_q = new TH1D("h1_mm_q","h1_mm_q",120,-20,10);
  TH1D *h1_mm_a = new TH1D("h1_mm_a","h1_mm_a",120,-20,10);
TH1D *h1_mm_a2= new TH1D("h1_mm_a2","h1_mm_a2",120,-20,10);
  TH1D *h1_mm   = new TH1D("h1_mm"  ,"h1_mm"  ,120,-20,10);
  TH1D *h1_mm2  = new TH1D("h1_mm2" ,"h1_mm2" ,120,-20,10);
  TH1D *h1_mm3  = new TH1D("h1_mm3" ,"h1_mm3" ,120,-20,10);
  SetTH1(h1_mm_p  ,"" ,"-B_{#Lambda} (MeV)","Counts / (250 keV)",kBlack,1,3004,kBlack);
  SetTH1(h1_mm_q  ,"" ,"-B_{#Lambda} (MeV)","Counts / (250 keV)",kRed,1,3004,kRed);
  SetTH1(h1_mm_a  ,"" ,"-B_{#Lambda} (MeV)","Counts / (250 keV)",kGreen ,1,1001,kGreen);
  SetTH1(h1_mm_a2 ,"" ,"-B_{#Lambda} (MeV)","Counts / (250 keV)",kGreen ,1,1001,kGreen);
  SetTH1(h1_mm    ,"" ,"-B_{#Lambda} (MeV)","Counts / (250 keV)",kAzure ,1,3004,kAzure);
  SetTH1(h1_mm2   ,"" ,"-B_{#Lambda} (MeV)","Counts / (250 keV)",kBlack ,1,3004,kBlack);
  SetTH1(h1_mm3   ,"" ,"-B_{#Lambda} (MeV)","Counts / (250 keV)",kBlack ,1,3004,kBlack);
...
//    double dE_max = 3.0;
    ...
    while(1){
      ...
      dE = gRandom->Uniform(0,3.);//0-1 MeV
      yy = dEdx(dE,Z,Me,P1);
      if(y<yy){ 
...
cout<<"Energy Loss"<<endl;
  ...
  c3_2->Divide(2,2,1E-4,1E-4);
  c3_2->cd(1);
  h2_dEep->Draw("colz");
  h2_dEep->Fit("[0]*sin(x)+[1]");
  c3_2->cd(2);
  h2_dEk->Draw("colz");
  h2_dEk->Fit("[0]*sin(x)+[1]");
  c3_2->cd(3);
  h2_2dEep->Draw("colz");
  h2_2dEep->Fit("gausn");
  c3_2->cd(4);
  ...
  c4->Divide(1,1,1E-4,1E-4);
	...
	TF1* ftot_core_result= new TF1("ftot_result",fmm_core_total,-20.,10.,64);
	...
	for(int i=0;i<19;i++){
		...
		}else if(i<14){ftot_core_result ->SetParameter(3*i,CS[i]*factor_p2);
...
//  double CS[19] = {  79.0,  26.3,  13.2,  35.1, CSpval1, CSpval1*100/320, CSpval1*60./320, CSpval1*100/320, CSpval1*60./320, CSpval2, CSpval2*100/320, CSpval2*60./320, CSpval2*100/320, CSpval2*60./320, CSpval3, CSpval3*100/320, CSpval3*60./320, CSpval3*100/320, CSpval3*60./320};
  ...
  //c1->Print("pdf/JPS2021fall/test.pdf");
  c1->Print("pdf/JPS2021fall/Mg27L_single_p532_250keVbin_fix_noscale.pdf");
  c4->Print("pdf/JPS2021fall/Mg27L_single_p532_250keVbin_fix_noscale_fit.pdf");
