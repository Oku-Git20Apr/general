...
void next_pepk()
{
  ...
  h2->SetYTitle("MM deviation [MeV/c^{2}]");
   TH2F *h2_pepk = new TH2F("h2_pepk","",5000.,1200.-150.,1200.+150.,5000,2740.-124.,2740.+124.);
...
//isomeric (theta_ee'=6.5\pm2.2 deg, theta_ek=11.5\pm2.6 deg)
	...
	double a = 2.*(Ee0+MP-Ek0)-2.*pe0*cos(te0)+2.*pk0*cos(te0+tk0);
    double b = (Ee0+MP-Ek0)*(Ee0+MP-Ek0)-pe0*pe0-pk0*pk0+2.*pk0*pe0*cos(te0)-ML*ML;
	double pep0 = b/a;
...
//isomeric (theta_ee'=6.5\pm2.2 deg, theta_ek=11.5\pm2.6 deg)
	...
	double a = 2.*(Ee0+MT-Ek0)-2.*pe0*cos(te0)+2.*pk0*cos(te0+tk0);
    double b = (Ee0+MT-Ek0)*(Ee0+MT-Ek0)-pe0*pe0-pk0*pk0+2.*pk0*pe0*cos(te0)-MH*MH;
	double pep0 = b/a;
	double Eep0 = sqrt(pep0*pep0+Me*Me);
