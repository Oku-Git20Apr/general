...
//  tree->SetBranchAddress("qdc", qdc); 
	...
	char condition[1000];
	sprintf(condition,"(qdc[15]-%f)/%f",pedestal,ope-pedestal);
	
    TH1D *h_qdc[2], *h_pe[2];
	TH2D *h2_qdc_tdc[2];
      h_qdc[0] = new TH1D("h_qdc[0]","h_qdc[0]",200,0.,800.);
	  tree->Project("h_qdc[0]","qdc[15]","","");
	  SetTH1(h_qdc[0],"Lead Glass QDC","QDC [ch]","Counts");
      h_qdc[1] = new TH1D("h_qdc[1]","h_qdc[1]",200,0.,800.);
	  tree->Project("h_qdc[1]","qdc[15]","tdc[15]>0","");
	  h_qdc[1]->SetLineColor(2);
//	  SetTH1(h_qdc[0],"Lead Glass QDC","QDC [ch]","Counts");
...
TCanvas *c[nCanvas];
    ...
    c[0]->Clear();
	c[0]->cd(1);h_qdc[0]->Draw("");h_qdc[1]->Draw("same");
