//RKVertex_ppi_extract.cc  2017.9.5 Y.Toyama
...
const double MaxCoinTime =  1.5;//IH-Tagger Coincidence Timing[ns]
const double MinCoinTime = -1.5;//IH-Tagger Coincidence Timing[ns]
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
struct TreeBranch{
...
void Initialization(){
  ...
  tr.vtype = -99;
  tr.dca   = -99;//distance between 2-tracks[cm]
  tr.ntr   = -99;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
class RKVertex_remakeroot : public RKVertex_tr
{
 public:
         RKVertex_remakeroot();
        ~RKVertex_remakeroot();
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
RKVertex_remakeroot::RKVertex_remakeroot()
{
  ParamMan = new RKV_ParamMan();
  MassRange_proton_min = 0.20;
  MassRange_proton_max = 2.5;
...
////////////////////////////////////////////////////////////////////////////
void RKVertex_remakeroot::SetParam(string ifname){
  input_param = ifname;
  ParamMan->read_param(input_param.c_str());
} 
////////////////////////////////////////////////////////////////////////////
void RKVertex_remakeroot::SetBranch(string ofname){
...
//vertex
  ...
  tree_out->Branch("dca"       , &tr.dca       , "DCA/D"          );
...
//IH
  ...
  tree_out->Branch("ih_dx"     , tr.ih_dx     , "ih_dx[3]/D"       );
...
////////////////////////////////////////////////////////////////////////////
void RKVertex_remakeroot::loop(){
cout<<"start loop"<<endl;
runmin=99999;runmax=-99;
bool ppi_flag;//proton & pi^{-} vertex
bool oa_flag, tag_flag, dedx_flag, ohseg_flag, vpos_flag;
...
int ppicounter=0;
  ...
  for(int n=0;n<ENum;n++){
    ...
    if(n%100000==0) cout<<n<<" / "<<ENum<< " ppi : "<<ppicounter<<endl;
    ...
    if(pi_id>-1){
      if(ihlr[pi_id]==-1){
        tdlmctime = tdllmctime[ihseg[pi_id]-1];
        tdltime_uw = tdllutime_w[ihseg[pi_id]-1];
        tdltime_dw = tdlldtime_w[ihseg[pi_id]-1];
      }
      else if(ihlr[pi_id]==1){
        tdlmctime = tdlrmctime[ihseg[pi_id]-1];
        tdltime_uw = tdlrutime_w[ihseg[pi_id]-1];
        tdltime_dw = tdlrutime_w[ihseg[pi_id]-1];
      }
      NHit = TagBHit(tagbctime, tdlmctime, seg);
    ...
    for(int i=0;i<ntr;i++){
      ...
      if(Proton(charge[i],mom[i],beta[i]) ) {p_id =i;}
      if(PiMinus(charge[i],mom[i],beta[i])) {pi_id=i;}

    }//for each track

    if(pi_id>-1&&p_id>-1&&ass_nih[pi_id]>0){
      ppi_flag=true;
      ppicounter++;
    }
...
/////////////////////////////////
    ...
    for(int i=0;i<3;i++){
      ...
      if(ihseg[i]>0){
        tr.ihct1[i]   = ihct1[i]  +ParamMan->GetT0(ihlr[i],1,ihseg[i]); 
        tr.ihct2[i]   = ihct2[i]  +ParamMan->GetT0(ihlr[i],1,ihseg[i]); 
        tr.ihct[i]    = ihct[i]   +ParamMan->GetT0(ihlr[i],1,ihseg[i]); 
      }
      else{
        tr.ihct1[i]   = ihct1[i]; 
        tr.ihct2[i]   = ihct2[i]; 
      ...
      if(ohseg[i]>0){
        tr.ohct1[i]   = ohct1[i]  +ParamMan->GetT0(ohlr[i],2,ohseg[i]); 
        tr.ohct2[i]   = ohct2[i]  +ParamMan->GetT0(ohlr[i],2,ohseg[i]); 
        tr.ohct[i]    = ohct[i]   +ParamMan->GetT0(ohlr[i],2,ohseg[i]); 
      }
      else{
        tr.ohct1[i]   = ohct1[i];
        tr.ohct2[i]   = ohct2[i];
    ...
    for(int i=0;i<40;i++){
      ...
      tr.tagbtime[i]    = tagbtime[i]   +ParamMan->GetT0(0,18,i+1);
      tr.tagbctime[i]   = tagbctime[i]  +ParamMan->GetT0(0,18,i+1);
    ...
    for(int i=0;i<NTDL;i++){
      ...
      tr.tdlluctime[i]  = tdlluctime[i]+ParamMan->GetT0(-1,15,i+1);
      tr.tdlructime[i]  = tdlructime[i]+ParamMan->GetT0( 1,15,i+1);
      tr.tdlldctime[i]  = tdlldctime[i]+ParamMan->GetT0(-1,15,i+1);
      tr.tdlrdctime[i]  = tdlrdctime[i]+ParamMan->GetT0( 1,15,i+1);
      tr.tdllmtime[i]   = tdllmtime[i] +ParamMan->GetT0(-1,15,i+1);
      tr.tdlrmtime[i]   = tdlrmtime[i] +ParamMan->GetT0( 1,15,i+1);
      tr.tdllmctime[i]  = tdllmctime[i]+ParamMan->GetT0(-1,15,i+1);
      tr.tdlrmctime[i]  = tdlrmctime[i]+ParamMan->GetT0( 1,15,i+1);
    }



    if(ppi_flag)tree_out ->Fill();
...
////////////////////////////////////////////////
void RKVertex_remakeroot::SetRoot(string ifname)
{
  cout<<"SetRoot"<<endl;
...
////////////////////////////////////////////////
void RKVertex_remakeroot::SetRootSingle(string ifname)
{
  cout<<"SetRoot"<<endl;
  add(ifname);
  readtree();
...
bool RKVertex_remakeroot::Proton(int charge, double mom, double beta){
...
bool RKVertex_remakeroot::PiMinus(int charge, double mom, double beta){

  //mom>0
  if(charge > 0){return false;}
  if(beta>1.){return true;}
  else{
    if(((mom*sqrt(1/pow(beta, 2.)-1.)) > MassRange_pion_min) &&
...
////////////////////////////////////////////////
int RKVertex_remakeroot::TagBHit(double *TagBTime, double IHTime, int *seg){
...
int main(int argc, char** argv){
  ...
  string pngname;
  extern char *optarg;
  while((ch=getopt(argc,argv,"hf:s:w:n:bcop:i:"))!=-1){
    switch(ch){
    case 'f':
      ifname = optarg;
  ...
  RKVertex_remakeroot *remake = new RKVertex_remakeroot();

  remake->SetMaxEvent(MaxNum);
  remake->SetParam(input_param);
  if(!single_rootfile_flag)remake->SetRoot(ifname);
  if( single_rootfile_flag)remake->SetRootSingle(ifname_root);
