#!/bin/csh -f
  ...
  if ( $#argv < 4 ) then
    echo ""
    echo "  Usage: ./Track.csh [period] [run number] [begin event] [end event]"
    echo ""
    exit
  endif
  ...
  set log_dir  = ${base_dir}job/log/Track/${run}/
  set root_dir = ${base_dir}job/root/Track/${run}/
  
  set host_num = `echo ${HOST} | sed 's/farm//g'`
  set job_tmp_dir  = /data/${host_num}c/.condor_job_tmp/${user_name}/Track_${period}_${run}_${bgnevt}_${endevt}
  ...
  set log_file    =     ${log_dir}/Track_${period}_${run}${seven_digit_bgnevt}${seven_digit_endevt}.log
  set conf_file   = ${job_tmp_dir}/Track_${period}_${run}${seven_digit_bgnevt}${seven_digit_endevt}.conf
  set output_file = ${job_tmp_dir}/Track_${period}_${run}${seven_digit_bgnevt}${seven_digit_endevt}.root


  set exec_file = ${base_dir}bin/TrackAnalysis
